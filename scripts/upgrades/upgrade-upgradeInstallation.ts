...
import { TileFacet__factory } from "../../typechain";
...
export async function upgrade() {
  ...
  const UpgradeQueue =
    "(uint256 parcelId, uint256 coordinateX, uint256 coordinateY, uint256 tileId, uint256 readyBlock, bool claimed, address owner)";

  const facets: FacetsAndAddSelectors[] = [
    {
      facetName: "TileFacet",
      addSelectors: [
        `function upgradeTile(${UpgradeQueue} _upgradeQueue) external`,
